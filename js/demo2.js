...
Tunnel.prototype.init = function() {
  ...
  this.mouse = {
	...
	light = new THREE.DirectionalLight(0x00000, 1);
  ...
  this.scene.fog = new THREE.Fog('#000d25',0.05,1.6);

  // Ast
  var light2 = new THREE.PointLight( '#6495ed', 6, 1000 );
  ...
  light3 = new THREE.PointLight( 0xff7f24, 6, 1000 );
  light3.position.set(250, 0, 750 );
  ...
  var light = new THREE.HemisphereLight( '#e9eff2', 0x017fe0, 1 );
  this.scene.add( light, light2, light3, pointCloud );
...
function Particle(scene, burst, spreadX, maxWidth, maxHeight, maxDepth, size) {
  ...
  if(burst){
  // Changed original color
  var color = '#6b452a';
  this.color = ColorLuminance(color,2+Math.random()*10);
  } else {
  // Changed original color
  var color = '#543524';
  this.color = ColorLuminance(color,2+Math.random()*10);
...
function createAsteroids(){
  ...
  var asteroids = [];
  for(var i=0;i<7;i++){
    asteroids.push(Particle(5+Math.random()*50,200,maxWidth,300,400));
  }
  for(var i=0;i<30;i++){
    asteroids.push(Particle(5+Math.random()*10,500,maxWidth,200,600));
  }
  for(var i=0;i<160;i++){
    asteroids.push(Particle(2+Math.random()*5,1000,maxWidth,150,800));
  }
  return asteroids;
}
...
Particle.prototype.sphere = new THREE.SphereBufferGeometry(1, 1, 2);
Particle.prototype.icosahedron = new THREE.IcosahedronBufferGeometry(1,0);
// Particle.prototype.cube = createAsteroids();
// Particle.prototype.sphere = createAsteroids();
// Particle.prototype.icosahedron = createAsteroids();
